...
def get_code_sections(name, align, offset=False):
    ...
    ## FIXME AO
    # print("NAME : ", name)
    # print("start address : ", section_start_address)
    
    # Skip is the section_start_address is empty
    if section_start_address: 
...
def AMI_get_sections_strings_from_partition(partitions):
    ...
    for r_id in sorted(partitions['Regions'].keys()):
        ...
        if region['Type'] == 'Code':
            ...
            if corresponding_data_section in partitions['Regions']:
                # print(corresponding_data_section)
