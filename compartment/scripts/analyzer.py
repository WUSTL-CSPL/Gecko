...
def merge_on_attr(G,attr=REGION_KEY):
    ...
    for n, attr in node_to_attr.items():
        ...
        if not attr in  attr_to_nodelist:
...
def add_region_to_comp_desc(R,r_node,comp_desc):
    ...
    r_type =R.nodes[r_node][TYPE_KEY]
    policy_r_type = r_node_to_comp_lut[r_type]
    ## print r_node
    ## print ("--------")
    ## print OBJECTS_KEY
    region = {POLICY_REGION_KEY_OBJECTS: R.nodes[r_node][OBJECTS_KEY],
...
def add_compartment_to_comp_desc(R,code_region,comp_desc):
    ...
    for p in R.predecessors(code_region):
        if R.nodes[p][TYPE_KEY] == DATA_REGION_KEY:
            data_regions.add(p)
        elif R.nodes[p][TYPE_KEY] == PERIPHERAL_REGION_KEY:
            per = ( R.nodes[p][BASE_ADDR_KEY],
                    R.nodes[p][PWR2_SIZE_KEY],
                    R.nodes[p][PRIV_KEY])
...
def get_mpu_region(T,tree_node,code_region,required_pers):
    ...
    if not KEY_REQUIRED_PERIPHERALS in mpu_attrs:
...
def optimize_filename_groupings(G,filename_to_code_nodes):
    ...
    for n,attrs in G.nodes(True):
        ...
        if attrs[TYPE_KEY] != FUNCTION_TYPE or not n in node_to_filenames:
            continue
...
def get_peripheral_nodes(G):
    ...
    p = filter(lambda nd: nd[1]['Type'] == 'PERIPHERAL_NODE_TYPE', G.nodes(data=True))
    return p
...
def data_compartment_puting(Region_Graph, filename, filename_to_data_nodes, node):
    ...
    for use_site in Region_Graph.neighbors(node):
        ...
        if not Region_Graph.node[use_site]["Filename"] in use_site_dic:
            # flag_shared = True
...
def data_compartment_puting_task(Region_Graph, controller, controller_to_data_nodes, node):
    ## print (Region_Graph.node[node])
    ## print(Region_Graph.node[node]["Filename"])
    ...
    for use_site in Region_Graph.neighbors(node):
        ...
        if "Controller" in Region_Graph.nodes[use_site]:
            use_site_controller = Region_Graph.nodes[use_site]["Controller"]

        #filename shoule be controller
        # if not use_site_dic.has_key(use_site_controller):
        if not use_site_controller in use_site_dic:
            # flag_shared = True
...
def add_quota(Region_Graph):
    for node in list(Region_Graph.nodes()):
        # Add quotes around the node name if it contains ':'
        if ':' in node:
            # Ensure the node name is a string
            new_node = f'"{node}"'
            # Relabel the node
            nx.relabel_nodes(Region_Graph, {node: new_node}, copy=False)

    # For attributes, you'll need to iterate over nodes and their attribute dict
    for node, data in Region_Graph.nodes(data=True):
        for attr_key in list(data):
            if ':' in str(data[attr_key]):
                # Add quotes around the attribute value
                data[attr_key] = f'"{data[attr_key]}"'

    # Now try to write to dot again
    nx.drawing.nx_pydot.write_dot(Region_Graph, "by_controller_code_merged_dot")



def partition_by_task(G, T):
    ...
    annotation_file_path = '/tmp/task_cmpt.txt'
    task_names = []
    ...
    print("Partioning by Task!")

    task_to_code_nodes = collections.defaultdict(list)
    task_to_data_nodes = collections.defaultdict(list)
    Region_Graph = G.copy()
    ...
    for (node, attrs) in Region_Graph.nodes(True):
        ...
        if TASK_TYPE in attrs:
            ...
            task = attrs[TASK_TYPE]

            if attrs[TYPE_KEY] == FUNCTION_TYPE:
                task_to_code_nodes[task].append(node)
                dic_fun_task[node] = task
            elif attrs[TYPE_KEY] == GLOBAL_TYPE:
                data_compartment_puting_task(Region_Graph, task, task_to_data_nodes, node)                
    
        else:

            if attrs[TYPE_KEY] == FUNCTION_TYPE:
                ### Check the connection to the target tasks
                connection_flag = False
                if "Connections" in attrs:
                    connections = attrs["Connections"]
                    ### Need to improve this code
                    for connection in connections:
                        for _task in task_to_code_nodes:
                            if _task in connection and not connection_flag:
                                # print(">>>>>>>>>>>>>>>>>>>")
                                task_to_code_nodes[_task].append(node)
                                connection_flag = True
                                break

                if not connection_flag:
                    if attrs[TYPE_KEY] == FUNCTION_TYPE:
                        task_to_code_nodes["other"].append(node)
                        dic_fun_task[node] = "other"

            ### If there is no connection to others
            elif attrs[TYPE_KEY] == GLOBAL_TYPE:
                data_compartment_puting_task(Region_Graph, "other", task_to_data_nodes, node) 
    print('-----------------------------------')

    # print("task_to_code_nodes : ", task_to_code_nodes)

    Region_Graph = build_regions_from_dict(Region_Graph, task_to_code_nodes, CODE_REGION_KEY)
    Region_Graph = AMI_build_data_regions_from_dict(Region_Graph, task_to_data_nodes, DATA_REGION_KEY)
    ...
    for node, data in Region_Graph.nodes(data=True):
        ...
        for attr_key, attr_value in data.items():
            new_attr_key = attr_key
            if ':' in attr_key:
                new_attr_key = f'"{attr_key}"'
            ...
            if ':' in str(attr_value):
                ...
                # TODO FIXME
                nodes_to_remove.append(node)
            new_data[new_attr_key] = attr_value
        
        nx.set_node_attributes(Region_Graph, {node: new_data})

        # Region_Graph.remove_node(node)
        # nx.relabel_nodes(Region_Graph, {node: new_node}, copy=False)

    for node in nodes_to_remove:
        if node in Region_Graph:
            Region_Graph.remove_node(node)
...
###################
    ...
    for node, data in Region_Graph.nodes(data=True):
        ...
        for attr_key in list(data):
            # print(node, "||||||||||||||||||||" , data)
            # print("@@@@@", attr_key)
            # if isinstance(data[attr_key], list):
            #     for item in data[attr_key]:
            #         if ':' in str(item):
            #             print("MMMMMMMMM", item)
            # else:
            #     print("@@@@@", data[attr_key])
...
def AMI_build_data_regions_from_dict(R,region_dict,r_type,r_id=0):
    ...
    for key, nodes in region_dict.items():
        ...
        if not (key == "share_data"):
            ...
            print("r_type : ", r_type)
            print("map_filename_to_id : ", map_filename_to_id)
            print("key : ", key)
...
def add_nodes(G,json_data,node_types):
    for node_name, node in json_data.items():
        ...
        if "Connections" in node:
            attrs["Connections"] = node["Connections"]
        # print("Hello whale : ", attrs)
...
def add_edges(G,json_data,edge_types):
    ...
    for node_name,node in json_data.items():
        ...
        for dest_name, con_info in node["Connections"].items():
            ...
            # if dic_weight.has_key(node_name):
            if node_name in dic_weight:
                # if dic_weight[node_name].has_key(dest_name):
                if dest_name in dic_weight[node_name]:
    ...
    for (n, attrs) in G.nodes(True):
        ...
        if attrs[TYPE_KEY] == PERIPHERAL_NODE_TYPE:
            ...
            if new_node:
                ...
                if not node_name in successors:
                    successors[node_name]=[]
                # if not predecessors.has_key(node_name):
                if not node_name in predecessors:
...
def best_partition(G,T,opt=True):
    ...
    for name in partition.keys():
        ...
        if G.node[name]["Type"] == FUNCTION_TYPE:
            ...
            if not region_id in partition_code_region:
            ...
            if not region_id in partition_data_region:
   ...
   # print "pre weight-based merge:" + str(len(partition_code_region))
    ...
    for key in dic_over_ivct:
        ...
        for des_name_in_ivct in dic_over_ivct[key]:
            ...
            if not des_name_in_ivct in partition:
            ...
            if not src_region_id == des_region_id:
                ...
                if not src_region_id in partition_code_region or not des_region_id in partition_code_region:
                    ## print ("-------")
                    ## print key
                    cnt_1 += 1
                    continue
...
if __name__ == '__main__':
    ...
    if args.outfile and args.partion_method:
        ...
        if args.linker_template and args.linker_output:
            # print("COMPT DEF: ", comp_def)
            ld_helpers.AMI_make_linker_script(args.linker_template,
                                          args.linker_output,
                                          comp_def)
            print("New link script generated!")
